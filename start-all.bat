...
#echo [9/12] Starting Notification Service (port 8090)...
#start "Notification Service - 8090" cmd /k "cd /d %BASE_DIR%notification-service && .\mvnw.cmd spring-boot:run"
#timeout /t 3 /nobreak >nul

echo [10/12] Starting Payment Service (port 900)...
start "Payment Service - 900" cmd /k "cd /d %BASE_DIR%payment-service && .\mvnw.cmd spring-boot:run"
timeout /t 3 /nobreak >nul

echo [11/12] Starting Latex Compiler Service (port 8091)...
start "Latex Compiler - 8091" cmd /k "cd /d %BASE_DIR%latex-compiler-service && .\mvnw.cmd spring-boot:run"
timeout /t 3 /nobreak >nul
...
echo.
echo ============================================================
echo    All 12 microservices and frontend are launching!
echo ============================================================
echo.
echo    Eureka Dashboard : http://localhost:8761
echo    API Gateway      : http://localhost:8080
echo    Latex UI         : http://localhost:5173
