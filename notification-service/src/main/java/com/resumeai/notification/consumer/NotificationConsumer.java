...
public class NotificationConsumer {
    ...
    public void handlePaymentSuccess(ConsumerRecord<String, Object> consumerRecord, Acknowledgment ack) {
        try {
            log.info("Received PAYMENT_SUCCESS event from partition {} offset {}", consumerRecord.partition(), consumerRecord.offset());
            PaymentSuccessEvent event = convert(consumerRecord.value(), PaymentSuccessEvent.class);
    ...
    @KafkaListener(topics = KafkaTopics.PAYMENT_FAILED, groupId = "notification-service-group",
                   containerFactory = "kafkaListenerContainerFactory")
    public void handlePaymentFailed(ConsumerRecord<String, Object> consumerRecord, Acknowledgment ack) {
        try {
            log.info("Received PAYMENT_FAILED event");
            PaymentFailedEvent event = convert(consumerRecord.value(), PaymentFailedEvent.class);
    ...
    @KafkaListener(topics = KafkaTopics.CREDIT_DEDUCTED, groupId = "notification-service-group",
                   containerFactory = "kafkaListenerContainerFactory")
    public void handleCreditDeducted(ConsumerRecord<String, Object> consumerRecord, Acknowledgment ack) {
        try {
            log.info("Received CREDIT_DEDUCTED event");
            CreditDeductedEvent event = convert(consumerRecord.value(), CreditDeductedEvent.class);
    ...
    @KafkaListener(topics = KafkaTopics.LOW_CREDITS_ALERT, groupId = "notification-service-group",
                   containerFactory = "kafkaListenerContainerFactory")
    public void handleLowCredits(ConsumerRecord<String, Object> consumerRecord, Acknowledgment ack) {
        try {
            log.info("Received LOW_CREDITS_ALERT event");
            LowCreditsEvent event = convert(consumerRecord.value(), LowCreditsEvent.class);
    ...
    @KafkaListener(topics = KafkaTopics.ATS_SCORE_RESULT, groupId = "notification-service-group",
                   containerFactory = "kafkaListenerContainerFactory")
    public void handleAtsScore(ConsumerRecord<String, Object> consumerRecord, Acknowledgment ack) {
        try {
            log.info("Received ATS_SCORE_RESULT event");
            AtsScoreEvent event = convert(consumerRecord.value(), AtsScoreEvent.class);
    ...
    @KafkaListener(topics = KafkaTopics.RESUME_ANALYSIS_RESULT, groupId = "notification-service-group",
                   containerFactory = "kafkaListenerContainerFactory")
    public void handleResumeAnalysis(ConsumerRecord<String, Object> consumerRecord, Acknowledgment ack) {
        try {
            log.info("Received RESUME_ANALYSIS_RESULT event");
            ResumeAnalysisEvent event = convert(consumerRecord.value(), ResumeAnalysisEvent.class);
    ...
    @KafkaListener(topics = KafkaTopics.USER_REGISTERED, groupId = "notification-service-group",
                   containerFactory = "kafkaListenerContainerFactory")
    public void handleUserRegistered(ConsumerRecord<String, Object> consumerRecord, Acknowledgment ack) {
        try {
            log.info("Received USER_REGISTERED event");
            UserRegisteredEvent event = convert(consumerRecord.value(), UserRegisteredEvent.class);
