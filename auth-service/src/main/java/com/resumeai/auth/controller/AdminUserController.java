...
public class AdminUserController {
    ...
    public ResponseEntity<Object> getUserById(@PathVariable int id) {
        return userAuthRepository.findById(id)
                .map(u -> ResponseEntity.ok((Object) new AdminUserDTO(
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Object> promoteToAdmin(@PathVariable int id) {
        return userAuthRepository.findById(id).map(u -> {
            ...
            return ResponseEntity.ok((Object) Map.of(MSG_KEY, USER_PREFIX + u.getUsername() + "' is now an ADMIN"));
        }).orElse(ResponseEntity.notFound().build());
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Object> demoteFromAdmin(@PathVariable int id,
                                              @RequestHeader("X-Username") String callerUsername) {
        return userAuthRepository.findById(id).map(u -> {
            if (u.getUsername().equals(callerUsername)) {
                return ResponseEntity.badRequest()
                        .body((Object) Map.of(MSG_KEY, "You cannot demote yourself"));
            }
            List<String> roles = new ArrayList<>(u.getRoles());
            ...
            return ResponseEntity.ok((Object) Map.of(MSG_KEY, "ADMIN role removed from '" + u.getUsername() + "'"));
        }).orElse(ResponseEntity.notFound().build());
    }
    ...
    @PostMapping("/{id}/ban")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Object> banUser(@PathVariable int id,
                                      @RequestHeader("X-Username") String callerUsername) {
        return userAuthRepository.findById(id).map(u -> {
            if (u.getUsername().equals(callerUsername)) {
                return ResponseEntity.badRequest()
                        .body((Object) Map.of(MSG_KEY, "You cannot ban yourself"));
            }
            u.setBanned(true);
            userAuthRepository.save(u);
            log.warn("Admin banned user '{}'", u.getUsername());
            return ResponseEntity.ok((Object) Map.of(MSG_KEY, USER_PREFIX + u.getUsername() + "' has been banned"));
        }).orElse(ResponseEntity.notFound().build());
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Object> unbanUser(@PathVariable int id) {
        return userAuthRepository.findById(id).map(u -> {
            u.setBanned(false);
            userAuthRepository.save(u);
            log.info("Admin unbanned user '{}'", u.getUsername());
            return ResponseEntity.ok((Object) Map.of(MSG_KEY, USER_PREFIX + u.getUsername() + "' has been unbanned"));
        }).orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<Object> deleteUser(@PathVariable int id,
        ...
        return userAuthRepository.findById(id).map(u -> {
            if (u.getUsername().equals(callerUsername)) {
                return ResponseEntity.badRequest()
                        .body((Object) Map.of(MSG_KEY, "You cannot delete your own account"));
            }
            userAuthRepository.delete(u);
            log.warn("Admin permanently deleted user '{}'", u.getUsername());
            return ResponseEntity.ok((Object) Map.of(MSG_KEY, USER_PREFIX + u.getUsername() + "' has been deleted"));
        }).orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<Map<String, Object>> getUserStats() {
        ...
        long admins = all.stream()
                .filter(u -> u.getRoles().contains(ROLE_ADMIN) || u.getRoles().contains("ROLE_ADMIN"))
                .count();
