...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    private OAuth2User handleGitHubEmail(OAuth2UserRequest userRequest, OAuth2User oauthUser) {
        String email = (String) oauthUser.getAttributes().get(EMAIL_ATTR);
        ...
        if (email == null || email.isBlank()) {
            ...
            if (fetchedEmail != null) {
                ...
                attributes.put(EMAIL_ATTR, fetchedEmail);
        ...
        try {
            ...
            if (emails != null && !emails.isEmpty()) {
                ...
                return emails.stream()
                    .filter(em -> Boolean.TRUE.equals(em.get("primary")) && Boolean.TRUE.equals(em.get("verified")))
                    .map(em -> (String) em.get(EMAIL_ATTR))
                    .findFirst()
                    // Or just the first verified one
                    .orElseGet(() -> emails.stream()
                        .filter(em -> Boolean.TRUE.equals(em.get("verified")))
                        .map(em -> (String) em.get(EMAIL_ATTR))
                        .findFirst()
                        // Or just the first one
                        .orElse((String) emails.get(0).get(EMAIL_ATTR)));
            }
        } catch (Exception _) {
            log.error("Failed to fetch email from GitHub API");
        }
        return null;
    }
}
